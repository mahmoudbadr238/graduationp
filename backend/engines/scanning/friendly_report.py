...
class FriendlyReportGenerator:
    ...
    def generate_file_report(
        ...
        if static_result:
            ...
            if static_result.get("groq_analysis"):
                checks.append("✓ AI malware classification")
        ...
        if static_result:
            ...
            # Groq AI analysis
            groq = _to_dict(static_result.get("groq_analysis", {}))
            if groq:
                item = {
                    "severity": "high" if int(groq.get("score", 0)) >= 70 else "medium",
                    "title": f"Groq AI: {groq.get('verdict', 'Unknown')}",
                    "detail": groq.get("explanation", "AI analysis completed"),
                    "source": "ai analysis",
                }
                key = (
                    str(item["severity"]).lower(),
