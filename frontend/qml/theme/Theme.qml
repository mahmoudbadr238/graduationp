...
QtObject {
    ...
    // ── Sync with SettingsService (Python backend) ──
    // On startup, read persisted values if SettingsService is already available.
    Component.onCompleted: {
        if (typeof SettingsService !== "undefined" && SettingsService) {
            themeMode = (SettingsService.themeMode || "dark").toString().trim().toLowerCase()
            fontSize  = SettingsService.fontSize || "medium"
        }
    }

    // Stay in sync whenever the Python side changes a value.
    property Connections _settingsConn: Connections {
        target: (typeof SettingsService !== "undefined" && SettingsService) ? SettingsService : null
        function onThemeModeChanged() {
            themeRoot.themeMode = (SettingsService.themeMode || "dark").toString().trim().toLowerCase()
            themeRoot.themeChanged()
        }
        function onFontSizeChanged() {
            themeRoot.fontSize = SettingsService.fontSize || "medium"
        }
    }
    ...
    function setThemeMode(mode) {
        themeMode = mode
        // Also persist through SettingsService
        if (typeof SettingsService !== "undefined" && SettingsService)
