...
Computes:
  ...
  - Optional Windows Defender scan (MpCmdRun.exe)
    - Optional Groq AI NGAV analysis
...
def _run_defender_scan(path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0 or "no threats found" in output.lower():
            return False, ""
        ...
        if result.returncode == 2:
            ...
            for line in output.splitlines():
                if "threat" in line.lower():
                    ...
                    break
            return True, threat or "Threat detected"

        # Unknown exit code — treat as scan error, not detection
        logger.warning("Defender exited with code %d for %s", result.returncode, path)
        return False, ""
...
def analyze_file_full(
    ...
    *,
    ...
    Windows Defender, Groq AI, and ClamAV.
    """
