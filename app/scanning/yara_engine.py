...
YARA_IMPORT_MODE = "fallback_patterns"
_YARA_BOOTSTRAP_ERROR = None
_YARA_MODE_LOGGED = False
...
def _load_native_yara_extension():
    ...
    if spec and spec.origin:
        ...
        site_packages = origin_path.parent
        if site_packages.name.lower() == "yara":
    ...
    candidates = sorted(site_packages.glob("yara*.pyd"))
    for candidate in candidates:
        try:
            native_spec = importlib.util.spec_from_file_location("yara", str(candidate))
            if not native_spec or not native_spec.loader:
                continue
            module = importlib.util.module_from_spec(native_spec)
            native_spec.loader.exec_module(module)
            sys.modules["yara"] = module
...
def _bootstrap_yara():
    ...
    try:
        ...
        YARA_AVAILABLE = True
        YARA_IMPORT_MODE = "legacy_wrapper"
    except (ImportError, OSError) as exc:
        _YARA_BOOTSTRAP_ERROR = exc
        YARA_AVAILABLE = False
        YARA_IMPORT_MODE = "fallback_patterns"
        yara = None
...
if not YARA_AVAILABLE and isinstance(_YARA_BOOTSTRAP_ERROR, ImportError):
...
class YaraEngine:
    ...
    def __init__(self, rules_dir: Path | None = None):
        ...
        global _YARA_MODE_LOGGED
        if not _YARA_MODE_LOGGED:
            logger.info(f"YARA mode selected: {YARA_IMPORT_MODE}")
            _YARA_MODE_LOGGED = True
