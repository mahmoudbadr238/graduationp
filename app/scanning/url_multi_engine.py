...
2. UrlScanner   — HTTP fetch, content analysis, YARA
...
from app.scanning.url_scanner import Evidence, UrlScanResult
...
_THREAT_KEYWORDS = {
    ...
    "malware": [
        "yara", "malware", "download attempt", "download content",
...
class MultiEngineUrlScanner:
    ...
    def scan(
        ...
        *,
        ...
        # ── 2. UrlScanner (HTTP fetch, content, YARA) ────────────────────
    ...
    def _run_checker(self, url: str) -> EngineResult:
        ...
        try:
            from app.scanning.url_checker import URLChecker
    ...
    def _run_scanner(
        ...
        """Run UrlScanner HTTP + content + YARA pipeline."""
        t = time.perf_counter()
        ...
        try:
            ...
            engine.details = {
                ...
                "yara_matches": len(scan_result.yara_matches),
                "ioc_count": (
    ...
    def _run_external_apis(self, url: str) -> list[EngineResult]:
        ...
        try:
            from app.scanning.url_external_apis import run_external_checks
    ...
    def _build_result(
        ...
        *,
        ...
        if final_score is None:
            try:
                from app.scanning.url_scoring import UrlScorer
                ...
                score_input = {
                    ...
                    "yara_matches": scan_result.yara_matches if scan_result else [],
        ...
        result: dict[str, Any] = {
            ...
            "yara_matches": [
                ...
                for m in (scan_result.yara_matches if scan_result else [])
            ],
            "yara_match_count": (
                len(scan_result.yara_matches) if scan_result and scan_result.yara_matches else 0
            ),
            # Signals
            "signals": scan_result.signals if scan_result else {},
            # Sandbox
