...
for d in disks:
    print(f"  {d['device']}: {d['percent']:.1f}% ({d['used']/(1024**3):.0f} GB / {d['total']/(1024**3):.0f} GB)")
    total_used += d['used']
...
if total_capacity > 0:
    ...
    print(f"Total: {total_used/(1024**3):.0f} GB / {total_capacity/(1024**3):.0f} GB ({len(disks)} drives)")
