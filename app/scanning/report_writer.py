...
def write_combined_scan_report(
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Generated:   {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("Report Type: Combined Static + Sandbox Analysis\n")
        f.write("Mode:        100% Offline (No data sent externally)\n\n")
        ...
        if sandbox_result:
            ...
            if sandbox_result.get("success"):
                ...
                if sandbox_result.get("timed_out"):
                    f.write("  Timed Out:      YES (possible evasion)\n")

                if sandbox_result.get("network_blocked"):
                    f.write("  Network:        BLOCKED\n")
