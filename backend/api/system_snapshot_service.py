...
class SystemSnapshotService(QObject):
        ...
        try:
            if True:  # Windows-only app
                ...
                if fw_status.get("status") == "Requires Admin":
                    ...
                    info["firewallName"] = fw_status.get("name", "")
                ...
                av_product_name = av_status.get("name", "") or "Antivirus"
                ...
                if av_status.get("status") == "requires_admin":
                    ...
                    info["antivirus"] = (
                        f"{av_product_name} (Real-time: {'On' if realtime else 'Off'})"
                    )
                    info["antivirusName"] = av_product_name
                else:
                    info["antivirusEnabled"] = False
                    info["antivirus"] = f"{av_product_name} (Disabled)"
                    info["antivirusName"] = av_product_name
                ...
                try:
                    ...
                    info["windowsUpdateLastInstall"] = win_update.get(
                        "lastInstallDate", ""
                    )
    ...
    def _push_security_notifications(self):
        ...
        if not internet.get("isGood"):
            issues.append(
                f"Internet Protection: {internet.get('status', 'Issue detected')}"
            )
        ...
        if not device.get("isGood"):
            issues.append(
                f"Device Protection: {device.get('status', 'Issue detected')}"
            )
