...
class BackendBridge(QObject):
    ...
    chatScanProgress = Signal(int, str)  # percent (0-100), status message
    ...
    def __init__(self):
        ...
        from backend.infra.nmap_cli import check_nmap_installed
    ...
    def _normalize_file_scan_result_for_qml(self, result: dict | None) -> dict:
        ...
        groq_analysis = normalized.get("groq_analysis")
        if not isinstance(groq_analysis, dict):
            groq_analysis = {}
        normalized["groq_analysis"] = groq_analysis
        ...
        if normalized.get("groq_ai_score") is None:
            normalized["groq_ai_score"] = int(groq_analysis.get("score", 0))
    ...
    def _generate_friendly_preview(
        ...
        if hasattr(self, "_event_explainer") and self._event_explainer:
            try:
                from backend.engines.ai.event_rules_engine import get_event_rules_engine
    ...
    def runNetworkScan(self, target: str, fast: bool = True):
        ...
        # Check cache first
        from backend.core.result_cache import ResultCache
    ...
    def scanFileLocal(self, path: str, run_sandbox: bool = False):
        ...
        - Groq AI NGAV metadata analysis
        - Optional ClamAV
        ...
        def scan_task(worker):
            ...
            from backend.engines.scanning import ReportWriter, SandboxController, StaticScanner
            ...
            return {
                ...
                "groq_ai_score": int((result.groq_analysis or {}).get("score", 0)),
        ...
        def on_success(wid, result):
            ...
            try:
                ...
                # ── Schema guardrail ────────────────────────────────────────────────
                try:
                    from backend.engines.scanning.report_schema import normalize_report_v2 as _nrv2
    ...
    def checkUrlLocal(self, url: str):
        ...
        def check_task(worker):
            ...
            from backend.engines.scanning import ReportWriter, URLChecker
    ...
    def openReportFolder(self, path: str = ""):
        ...
        from backend.engines.scanning import ReportWriter
    ...
    def sandboxAvailable(self) -> bool:
        ...
        try:
            from backend.engines.scanning import SandboxController
    ...
    def integratedSandboxAvailable(self) -> bool:
        ...
        try:
            from backend.engines.scanning.integrated_sandbox import get_integrated_sandbox
    ...
    def runIntegratedScan(
        ...
        1. Static analysis (PE, entropy, hashes, Groq AI, IOCs)
        ...
        def scan_task(worker):
            ...
            from backend.engines.scanning.friendly_report import get_friendly_report_generator
            from backend.engines.scanning.integrated_sandbox import get_integrated_sandbox
            ...
            if run_sandbox:
                ...
                try:
                    from backend.engines.scanning.sandbox_session import SandboxSession
                    ...
                    if avail.get("available"):
                        ...
                        from backend.engines.scanning.sandbox_session import SandboxEvent
            ...
            result = {
                ...
                "groq_ai_score": int(
                    (static_result.get("groq_analysis", {}) or {}).get("score", 0)
                ) if static_result else 0,
                "groq_analysis": static_result.get("groq_analysis", {})
                if static_result
                else {},
        ...
        def on_success(wid, result):
            ...
            try:
                ...
                try:
                    ...
                    from backend.engines.scanning.report_schema import (
                        build_empty_report,
                        save_report,
                        score_to_label,
                        score_to_risk,
                    ...
                    _sr["static"]["groq_analysis"] = result.get("groq_analysis", {})
    ...
    def _start_live_preview(
        ...
        try:
            from backend.engines.scanning.window_capture import WindowCaptureService
    ...
    def _start_autopilot(self):
        ...
        try:
            from backend.engines.scanning.installer_autopilot import InstallerAutopilot
    ...
    def urlSandboxAvailable(self) -> bool:
        ...
        try:
            from payload.url_detonator.webview2_detonator import check_webview2_available
    ...
    def _run_url_scan(
        ...
        def scan_task(worker):
            ...
            try:
                from backend.engines.scanning.url_multi_engine import MultiEngineUrlScanner
    ...
    def runNmapScan(self, scan_type: str, target_host: str) -> None:
        ...
        from backend.core.workers import WorkerWatchdog
        from backend.infra.nmap_cli import (
            SCAN_PROFILES,
            NmapCli,
            get_local_subnet,
            get_reports_dir,
    ...
    def exportNmapScanReport(self, scan_id: str) -> None:
        ...
        from backend.infra.nmap_cli import get_reports_dir
    ...
    def startSandboxFileAnalysis(self, file_path: str) -> None:
        ...
        from backend.engines.sandbox.analyzer_dynamic import run_file_analysis
        ...
        def _run() -> None:
            try:
                ...
                try:
                    ...
                    from backend.engines.sandbox.report_explainer import (
        ...
        *options_json* is a JSON object:
        ...
        def _run() -> None:
            ...
            try:
                ...
                if opts.use_sandbox:
                    try:
                        from backend.engines.sandbox_vmware.preview_stream import SandboxPreviewStream
                        from backend.engines.sandbox.vmware_runner import load_runner_config
                        import os as _os
                ...
                try:
                    from backend.api.notification_service import get_notification_service as _get_ns
                    _ns = _get_ns()
                    _v   = report.verdict
                    _score = _v.score  if _v else 0
    ...
    def explainScanCenterReport(self, report_json: str) -> None:
        ...
        from backend.engines.scancenter.groq_explainer import explain_report
        from backend.engines.scancenter.report_schema import V3Report
    ...
    def openScanCenterReport(self, report_path: str) -> None:
        ...
        from backend.engines.scancenter.export import load_report_json
