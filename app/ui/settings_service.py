...
class SettingsService(QObject):
    ...
    def _load_settings(self):
        ...
        try:
            if self._settings_path.exists():
                ...
                self._update_interval_ms = data.get("updateIntervalMs", self._update_interval_ms)
                self._enable_gpu_monitoring = data.get("enableGpuMonitoring", self._enable_gpu_monitoring)
                self._start_minimized = data.get("startMinimized", self._start_minimized)
                self._start_with_system = data.get("startWithSystem", self._start_with_system)
                self._send_error_reports = data.get("sendErrorReports", self._send_error_reports)
    ...
    def networkUnit(self, value: str):
        if value in ["auto", "bps", "Kbps", "Mbps", "Gbps"] and value != self._network_unit:
