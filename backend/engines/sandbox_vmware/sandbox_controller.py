...
class SandboxLabController(QObject):
    ...
    def _task_run_file(self, worker: VmwareTaskWorker) -> dict[str, Any]:
        ...
        # ── Static analysis on host (PE, Groq AI, hashes, entropy, IOCs) ───
        static_info: dict[str, Any] = {}
        worker.emit_step(
            "Running", "Static analysis: hashing, PE, Groq AI, IOC extraction…"
        )
        try:
            ...
            groq_score = int((raw.get("groq_analysis") or {}).get("score", 0))
        ...
        if static_info:
            ...
            groq_score = int((static_info.get("groq_analysis") or {}).get("score", 0))
            ...
            boost = min(30, int(groq_score * 0.25) + ioc_count * 3)
            if boost:
                ...
                if groq_score:
                    highlights.insert(
                        0,
                        "Groq AI: host static scan returned "
                        f"{groq_score}/100 risk score.",
                    )
                if ioc_count:
                    highlights.insert(
        ...
        try:
            ...
            from backend.engines.scanning.report_schema import (  # type: ignore[import]
                build_empty_report,
                report_dir,
                save_report,
                score_to_label,
            ...
            _st["groq_analysis"] = dict(static_info.get("groq_analysis") or {})
    ...
    def _task_run_pipeline(
        ...
        if not report_path.exists() and self._run_dir is not None:
            try:
                ...
                from backend.engines.scanning.report_schema import save_report as _save_report
    ...
    def _sync_capture_state(self) -> None:
        ...
        if should_capture and not self._capture_timer.isActive():
            ...
            if self._preview_stream is None or not self._preview_stream.running:
                ...
                _frame_cb = None
                try:
                    from backend.api.sandbox_preview_provider import get_preview_provider, get_preview_controller
            ...
            try:
                from backend.api.sandbox_preview_provider import get_preview_controller
        ...
        def job() -> None:
            try:
                ...
                if not abort_ev.is_set():
                    ...
                    try:
                        from backend.api.sandbox_preview_provider import get_preview_provider
