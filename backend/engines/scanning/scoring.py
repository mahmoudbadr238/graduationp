...
class ThreatScorer:
    ...
    WEIGHTS = {
        ...
        "groq_ai_weight": 0.5,  # Blend factor for Groq AI score contribution
        "suspicious_import": 5,  # Per suspicious import
        "suspicious_import_max": 25,  # Cap for import scoring
        "packed_binary": 10,  # Entropy suggests packing
    ...
    def _score_static(self, result: dict[str, Any]) -> int:
        ...
        if isinstance(groq, dict) and groq:
            groq_score = max(0, min(100, int(groq.get("score", 0))))
            groq_pts = int(groq_score * self.WEIGHTS["groq_ai_weight"])
            points += groq_pts
            self._breakdown["groq_ai"] = groq_pts
            self._explanations.append(
                "Groq AI classified the PE metadata as "
                f"{groq.get('verdict', 'Unknown')} ({groq_score}/100)."
            )
