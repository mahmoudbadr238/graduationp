...
Covers:
    ...
    - backend.engines.sandbox.engines        (EngineResult structure)
    - backend.engines.sandbox.report_builder (_compute_verdict helper)
    - backend.engines.sandbox.report_explainer (_offline_fallback)
...
for _m in ("psutil", "PySide6", "PySide6.QtCore", "groq"):
    _stub(_m)
...
class TestScoreToRisk(unittest.TestCase):
    def setUp(self) -> None:
        self.mod = importlib.import_module("backend.engines.sandbox.report_schema")
...
class TestEngineResultStructure(unittest.TestCase):
    def setUp(self) -> None:
        self.mod = importlib.import_module("backend.engines.sandbox.engines")
    ...
    def test_groq_ai_engine_returns_structured_result(self) -> None:
        """Groq AI engine wrapper should return a valid EngineResult shape."""
        result = self.mod.run_groq_ai(Path(__file__))
        self.assertIn(result["status"], {"clean", "suspicious", "malicious", "error", "not_installed"})
        self.assertEqual(result["name"], "GroqAI")
...
class TestComputeVerdict(unittest.TestCase):
    def setUp(self) -> None:
        self.mod = importlib.import_module("backend.engines.sandbox.report_builder")
    ...
    def test_clean_verdict(self) -> None:
        engines = [
            ...
            {"name": "GroqAI", "status": "clean", "details": ""},
    ...
    def test_malicious_verdict(self) -> None:
        ...
        engines = [
            ...
            {"name": "GroqAI", "status": "malicious", "details": "groq-ai-flag-001"},
            {"name": "ClamAV", "status": "not_installed", "details": ""},
        ]
        v = self._cv(engines)
    ...
    def test_suspicious_verdict(self) -> None:
        engines = [
            {"name": "Defender", "status": "clean", "details": ""},
            {"name": "GroqAI", "status": "suspicious", "details": "generic"},
            {"name": "ClamAV", "status": "not_installed", "details": ""},
        ]
        v = self._cv(engines)
        # 15 pts from one suspicious engine hit → Low/Medium boundary
...
class TestOfflineFallback(unittest.TestCase):
    def setUp(self) -> None:
        self.mod = importlib.import_module("backend.engines.sandbox.report_explainer")

    def _make_report(self, risk: str, score: int) -> dict:
        from backend.engines.sandbox.report_schema import build_empty_report
